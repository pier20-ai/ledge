...
`blocks` are exercise apps. (`focus` retired to the archive at G2.9 — "not
...
predates the design reset — `chess` and `blocks` were rebuilt *out* of it in
...
| `meta` | `{ name?, icon?, panel?, settings? }` | Catalog identity. `icon` is an SF Symbol as `"sf:name"`. `panel` is `{ width?, maxHeight? }` in points — `maxHeight` is ordinary; `width` is the exception (see "Layout and size") and no default app declares one. `settings` declares native controls — see below. |
...
The root is a fixed width: **480 pt**, the same for every app (G6). The shell's
own controls stand at the panel's two top corners, so the glass has to be one
known size for them to stand still — and it means you lay out against a width
you know. `meta.panel.width` survives as an exception for a well that is
genuinely worth more glass: it can only ask for *more* (a smaller request is
480), the shell clamps it to the screen, and the controls move out to the wider
edges. Do not declare one to get a narrower panel — centre your content in the
480 instead, as `blocks` does. None of the default apps declares a width.
Height is measured from your tree and clamped. Content past the clamp
**clips** — there is no implicit scrolling; opt in with `<stack scroll>`.
...
| `image` | `src` (an absolute file path), `x`, `y`, `w`, `h`, optional `radius` (clips to a rounded rect), and `sx`/`sy`/`sw`/`sh` to draw one cell of a spritesheet, in image pixels from the top-left |
